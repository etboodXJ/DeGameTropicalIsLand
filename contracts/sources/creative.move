...
// 创意类型枚举
public enum CreativeType has copy, drop, store {
    // 数字内容类
    ImageText, // 1. 图文创意
    Video, // 2. 视频创意
    Novel, // 3. 小说
...
public fun creative_type_to_string(creative_type: &CreativeType): String {
    match (creative_type) {
        CreativeType::ImageText => string::utf8(b"图文创意"),
        CreativeType::Video => string::utf8(b"视频创意"),
        CreativeType::Novel => string::utf8(b"小说"),
        CreativeType::ProductCrowdFund => string::utf8(b"产品众筹"),
        CreativeType::DappSoftware => string::utf8(b"DAPP软件"),
        CreativeType::Game => string::utf8(b"游戏"),
        CreativeType::MobileApp => string::utf8(b"移动应用"),
        CreativeType::Website => string::utf8(b"网站"),
        CreativeType::PhysicalStore => string::utf8(b"实体店"),
        CreativeType::OfflineEvent => string::utf8(b"线下活动"),
        CreativeType::OfflineExhibition => string::utf8(b"线下展览"),
        CreativeType::OfflinePerformance => string::utf8(b"线下演出"),
        CreativeType::OfflineLecture => string::utf8(b"线下讲座"),
        CreativeType::OfflineTraining => string::utf8(b"线下培训"),
        CreativeType::OfflineCompetition => string::utf8(b"线下比赛"),
        CreativeType::OfflineGathering => string::utf8(b"线下聚会"),
    }
}
...
public fun string_to_creative_type(type_str: &String): CreativeType {
    let image_text = string::utf8(b"图文创意");
    let video = string::utf8(b"视频创意");
    let novel = string::utf8(b"小说");
    let product_crowd_fund = string::utf8(b"产品众筹");
    let dapp_software = string::utf8(b"DAPP软件");
    let game = string::utf8(b"游戏");
    let mobile_app = string::utf8(b"移动应用");
    let website = string::utf8(b"网站");
    let physical_store = string::utf8(b"实体店");
    let offline_event = string::utf8(b"线下活动");
    let offline_exhibition = string::utf8(b"线下展览");
    let offline_performance = string::utf8(b"线下演出");
    let offline_lecture = string::utf8(b"线下讲座");
    let offline_training = string::utf8(b"线下培训");
    let offline_competition = string::utf8(b"线下比赛");
    let offline_gathering = string::utf8(b"线下聚会");
...
public fun create_creative(
    ...
    Creative {
        ...
        tags,
        category,
        encrypted_id: string::utf8(b""), // 初始化为空字符串，后期可设置加密ID
        follow: @0x0, // 初始化为零地址
        prev: @0x0, // 初始化为零地址
...
// 提交创意审核
public fun submit_creative(creative: &mut Creative, ctx: &TxContext) {
    // 验证创作者
...
// 审核创意 (管理员功能)
public fun review_creative(creative: &mut Creative, approved: bool, ctx: &TxContext) {
    // 验证状态
...
// 发布创意
public fun publish_creative(creative: &mut Creative, ctx: &TxContext) {
    // 验证创作者
    assert!(creative.creator == ctx.sender(), 1);
...
// 删除创意
public fun delete_creative(creative: Creative, ctx: &TxContext) {
    // 验证创作者
    assert!(creative.creator == ctx.sender(), 1);
...
// 转移创意实例
public fun transfer_instance(instance: &mut CreativeInstance, new_owner: address, ctx: &TxContext) {
    // 验证当前所有者
    assert!(instance.owner == ctx.sender(), 1);
...
public fun update_expectation(
    ...
    // 验证用户
    assert!(expectation.user == ctx.sender(), 1);
...
// 计算用户期待值占比
public fun calculate_user_expectation_ratio(creative: &Creative, user_expectation: u64): u64 {
...
// 更新创意收入
public fun update_revenue(creative: &mut Creative, amount: u64, ctx: &TxContext) {
...
// 设置加密ID
public fun set_encrypted_id(creative: &mut Creative, encrypted_id: String, ctx: &TxContext) {
    // 验证创作者
    assert!(creative.creator == ctx.sender(), 1);
...
// 创建创意统计
public fun create_stats(creative_id: ID, ctx: &mut TxContext): CreativeStats {
    CreativeStats {
        id: object::new(ctx),
        creative_id,
        total_views: 0,
...
public fun update_stats(
    ...
    stats.last_updated = ctx.epoch();
}

// 获取统计信息
public fun get_stats(stats: &CreativeStats): (u64, u64, u64, u64) {
    (stats.total_views, stats.total_purchases, stats.total_expectations, stats.average_rating)
}
...
// Entry 函数：创建并共享创意对象
public entry fun create_and_share_creatives(ctx: &mut TxContext) {
...
// 添加创意到共享对象
public fun add_creative_to_shared(shared: &mut SharedCreatives, creative_id: ID, _ctx: &TxContext) {
...
// 外部调用入口函数 - 提交创意到共享对象
public entry fun submit_creative_to_shared(
    title: String,
    description: String,
    content: String,
    creative_type: u8,
    ...
    let creative = Creative {
        ...
        tags,
        category,
        encrypted_id: string::utf8(b""), // 初始化为空字符串
        follow: @0x0, // 初始化为零地址
        prev: @0x0, // 初始化为零地址
        fans: vector::empty<address>(), // 初始化为空向量
        list: vector::empty<address>(), // 初始化为空向量
...
// 标记删除共享创意
public entry fun mark_creative_as_deleted(
    creative: &mut Creative,
    clock: &Clock,
    ctx: &mut TxContext,
) {
    ...
    creative.updated_at = clock.timestamp_ms();
}
...
// Entry函数：修改创意分类
public entry fun update_creative_category_entry(
    creative: &mut Creative,
    new_category: String,
    ctx: &mut TxContext,
) {
    update_creative_category(creative, new_category, ctx);
}
...
public fun get_creative_detail_info(
    detail: &CreativeDetail,
): (String, String, String, u8, address, u64) {
    (
        detail.title,
        detail.description,
        detail.category,
...
public fun update_creative_detail(
    ...
    // 验证创作者
    assert!(detail.creator == ctx.sender(), 1);
    ...
    detail.updated_at = ctx.epoch();
}

// 提交 CreativeDetail 审核
public fun submit_creative_detail(detail: &mut CreativeDetail, ctx: &TxContext) {
    // 验证创作者
    assert!(detail.creator == ctx.sender(), 1);
...
public fun review_creative_detail(detail: &mut CreativeDetail, approved: bool, ctx: &TxContext) {
    // 验证状态
...
// 发布 CreativeDetail
public fun publish_creative_detail(detail: &mut CreativeDetail, ctx: &TxContext) {
    // 验证创作者
    assert!(detail.creator == ctx.sender(), 1);
...
// 删除 CreativeDetail
public fun delete_creative_detail(detail: CreativeDetail, ctx: &TxContext) {
    // 验证创作者
    assert!(detail.creator == ctx.sender(), 1);
...
// 添加粉丝到 Creative
public fun add_fan_to_creative(creative: &mut Creative, fan_address: address, ctx: &TxContext) {
    // 验证创意状态
    assert!(creative.status == STATUS_PUBLISHED, 1);
...
// 设置 Creative 的跟随地址
public fun set_creative_follow(creative: &mut Creative, follow_address: address, ctx: &TxContext) {
    // 验证创作者
    assert!(creative.creator == ctx.sender(), 1);
...
public fun set_creative_prev(creative: &mut Creative, prev_address: address, ctx: &TxContext) {
    // 验证创作者
    assert!(creative.creator == ctx.sender(), 1);
...
public fun get_category_idea(): String {
    string::utf8(CATEGORY_IDEA)
}

public fun get_category_prototype(): String {
    string::utf8(CATEGORY_PROTOTYPE)
}

public fun get_category_project(): String {
    string::utf8(CATEGORY_PROJECT)
}

public fun get_category_resource(): String {
    string::utf8(CATEGORY_RESOURCE)
}

// 验证分类是否有效
public fun is_valid_category(category: &String): bool {
    let idea = string::utf8(CATEGORY_IDEA);
    let prototype = string::utf8(CATEGORY_PROTOTYPE);
    let project = string::utf8(CATEGORY_PROJECT);
    let resource = string::utf8(CATEGORY_RESOURCE);
