...
const MyPointsPage = () => {
  ...
  const platforms = {
    bucket: { name: 'Bucket Protocol', icon: '🪣', color: 'blue' },
    cetus: { name: 'Cetus Protocol', icon: '🐋', color: 'purple' },
    navi: { name: 'Navi Protocol', icon: '🧭', color: 'green' }
  };
    ...
    <Box className="min-h-screen">
      ...
      <Container className="container mx-auto p-6">
        ...
        <Flex align="center" gap="4" className="mb-8">
          ...
          <Heading as="h1" size="6" className="text-white">我的积分记录</Heading>
        </Flex>

        {/* 积分统计 */}
        <Flex className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Box className="glass rounded-xl p-6 text-center">
            <Text size="2" className="text-gray-400 mb-2">当前积分</Text>
            <Text size="6" className="text-yellow-400 font-bold">
              {points.toLocaleString()}
            </Text>
            <Text size="2" className="text-gray-400">CYKJ</Text>
          </Box>
          
          <Box className="glass rounded-xl p-6 text-center">
            <Text size="2" className="text-gray-400 mb-2">累计存款</Text>
            <Text size="6" className="text-blue-400 font-bold">
              {totalDeposited.toFixed(2)}
            </Text>
            <Text size="2" className="text-gray-400">SUI</Text>
          </Box>
          
          <Box className="glass rounded-xl p-6 text-center">
            <Text size="2" className="text-gray-400 mb-2">累计获得</Text>
            <Text size="6" className="text-green-400 font-bold">
        ...
        <Box className="glass rounded-xl p-6">
          <Heading as="h2" size="4" className="text-white mb-6">交易记录</Heading>
          
          {records.length === 0 ? (
            <Box className="text-center py-12">
              <Text size="6" className="mb-4 block">📊</Text>
              <Text className="text-gray-400 mb-4">暂无积分记录</Text>
              <button 
                onClick={() => navigate('/points')}
                    ...
                    <Flex justify="between" align="center">
                      <Flex align="center" gap="4">
                        <Box className="text-center">
                          <Text size="4">{platform.icon}</Text>
                        </Box>
                        <Box>
        ...
        <Box className="glass rounded-xl p-6 mt-8">
          <Heading as="h3" size="3" className="text-white mb-4">积分使用说明</Heading>
